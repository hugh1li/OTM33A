...
importSTR <- function(STRfile, lagfile, ID, data.names) {
  # Inputs
...
#  library(readxl)
  ...
  if (ftype == ".xls") {
    ...
    rawdat <- read.table(STRfile, header=FALSE, skip=numskip)
    names(rawdat)<- data.names
    rawdat$time <- as.POSIXct(strptime(paste(rawdat[, "Date"],rawdat[,"Time"],sep=" "), 
                                       format = "%m/%d/%y %H:%M:%S"))
  }
  ...
  names(rawdat)[grep("Wind.Direction", names(rawdat))] <- "WS.Wind.Direction"
  ...
  # RA: rotate 180, take arctan, rotate back
